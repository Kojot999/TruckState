...
const initialVehicleGroups = [
  ...
  { id: 'g2', name: 'Oddział Poznań' },
  { id: 'g3', name: 'Oddział Legnica' },
  { id: 'g4', name: 'Oddział Warszawa' },
  { id: 'g5', name: 'Oddział Kraków' },
  { id: 'g6', name: 'Oddział Gdańsk' },
];
...
function seedFleetData() {
  writeToStorage(STORAGE_KEYS.vehicles, initialVehicles);
  writeToStorage(STORAGE_KEYS.posts, initialPosts);
...
export function initMockData() {
  ...
  if (normalizedVehicles.some((vehicle, index) => vehicle.type !== vehicles[index].type)) {
    writeToStorage(STORAGE_KEYS.vehicles, normalizedVehicles);
  } else if (vehicles.some((vehicle) => vehicle.groupId === undefined)) {
    writeToStorage(
      STORAGE_KEYS.vehicles,
      vehicles.map((vehicle) => ({ ...vehicle, groupId: vehicle.groupId ?? null }))
    );
  }
}
